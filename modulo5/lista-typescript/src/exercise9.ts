...
Outra regra muito importante é que CPFs com números iguais como: `111.111.111-11`, `222.222.222-22`, entre outros, são CPFs válidos pelo algoritmo 
mas não existem no registro oficial. Assim esse tipo de CPF não pode ser usado.

 
Você deve criar uma função que receba o CPF no formato “xxx.xxx.xxx-xx” e faça uma validação do valor recebido. Caso o CPF recebido 
seja válido retorne um `True` e caso seja inválido retorne `False` */
...
const isValid = (cpf: string) => {
    ...
    let multiplicator: number = 2;
    let result;
    
    for(let i = 0; i < cpfArray.length; i++){
        cpfArrayNumbers.push(parseInt(cpfArray[i]))
    }
...
console.log(isValid("145.382.206-20"))